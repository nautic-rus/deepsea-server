select issue.id, issue.status, issue.doc_number,
       ...
       min (case when issue_combined.issue_first = issue.id and i2.issue_type = 'CORRECTION' and i2.status not in ('Delivered') and i2.removed = 0 then i2.due_date end) as max_due_date
from issue
