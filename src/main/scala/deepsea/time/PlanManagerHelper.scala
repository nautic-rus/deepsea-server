    ...
    if (tasks.nonEmpty){
      ...
      if (consumedByTask.nonEmpty) {
        ...
        tasks.foreach(t => {
          ...
          hours ++= getHoursOfIntervalWithSkip(t.date_start, t.date_finish, skip)
        })
//        val caltoday = Calendar.getInstance()
//        caltoday.set(caltoday.get(Calendar.YEAR), caltoday.get(Calendar.MONTH), caltoday.get(Calendar.DAY_OF_MONTH), 8, 0, 0)
...
//          })
          val skip = tasks.map(_.user_id).distinct.flatMap(skipIntervals)
  ...
  def insertInterval(taskId: Int, userId: Int, from: Long, hoursAmount: Int, taskType: Int, consumed: Int = 0, min: Long = 0): Int = {
    ...
    else if (taskType == 0 && taskUsers.nonEmpty && !taskUsers.contains(userId) && consumedByTask < amountInPlan){
      -5
    }
...
//    }
  ...
  private def splitTask(splitDate: Long, userId: Int, removeAmount: Int = 0, allowAnyUser: Boolean = false): List[PlanInterval] = {
    DBManager.GetPGConnection() match {
      case Some(c) =>
        val s = c.createStatement()
  ...
  private def getHoursOfIntervalWithSkip(dateStart: Long, dateFinish: Long, skip: List[PlanInterval]): List[Long] = {
    val res = ListBuffer.empty[Long]
    var hour = dateStart
    while (hour <= dateFinish){
      res += hour
      hour = nextHourWithSkip(hour, skip)
    }
    res.toList
  }
