...
class ActorStartupManager extends Actor{
  override def receive: Receive = {
    ...
    case HTTPManagerStarted() =>
      ...
      ActorManager.issue = system.actorOf(RoundRobinPool(20).props(Props[IssueManager]))
      ActorManager.files = system.actorOf(RoundRobinPool(20).props(Props[FileManager]))
