    ...
    GetConnection() match {
      case Some(c) =>
        ...
        checks.foreach(check => {
          s.execute(s"insert into issue_check (issue_id, check_description, user_login, check_date, check_status, check_group) values ($issue_id, '${check.check_description}', '', 0, 0, '${check.check_group}')")
        })
        s.close()
        c.close()
      case _ =>
